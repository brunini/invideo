...
if [ $ARG = '-m' ] || [ $ARG = '-f' ]; then
    ...
    echo 'You must specify an first argument, use -h to see the documentation.'
    exit 0
fi
...
if [ $ARG = '-h' ]; then
    echo '
    Usage: invideo [OPTION] [TARGET] [DESTINATION_FOLDER]
...
echo "Searching for videos in $TARGET."
...
do
    EXISTS=0
    echo ''
    echo "Getting file hash of $FILE_PATH"
    ...
    if [ $EXISTS -eq 1 ]; then
        echo $SH "exists in list, avoiding copy."
    else
        echo $SH "not exists in list, copying."
